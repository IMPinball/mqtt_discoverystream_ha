...
When configuring the Home Assistant MQTT integration on the instance running MQTT DiscoveryStream (the "publisher" instance), it is crucial to prevent a feedback loop that leads to entity proliferation.

Preventing Self-Duplication: To avoid a recursive loop where the publisher Home Assistant instance discovers entities it itself is publishing, you must disable discovery on the local Home Assistant MQTT integration. Failure to do so will result in an exponential increase of duplicate entities (e.g., _2, _2_2, _2_2_2) in your Home Assistant instance.

Navigate to Settings > Devices & Services > MQTT integration.

Click the "Configure" gear.

<img width="931" height="1055" alt="mqtt configuration - Copy" src="https://github.com/user-attachments/assets/66106e88-30b7-4d56-b182-a8b3788462a4" />

Click "Configure MQTT Options"

<img width="897" height="1329" alt="mqtt settings - Copy" src="https://github.com/user-attachments/assets/cf1295b5-1113-4662-bba9-6398221f0dcb" />

Under "Discovery prefix", change the value to something unique and that will not be listened to by Home Assistant, such as _no_discovery_here_. This ensures the local instance's MQTT integration ignores the discovery messages generated by MQTT DiscoveryStream, you may also untick the enable discovery option here.

Recommended Topic Separation: It is highly beneficial to separate the MQTT Discovery topic from the default Home Assistant base_topic used for state updates. This provides clearer topic management and helps prevent potential conflicts.

In your mqtt_discoverystream configuration (e.g., in configuration.yaml), explicitly set a distinct discovery_topic:

```yaml

mqtt_discoverystream_alt:
  base_topic: homeassistant                   # Used for state updates
  discovery_topic: homeassistant_discovery    # Used exclusively for discovery messages
...
Ensure Retained Discovery Messages: For reliable discovery across Home Assistant restarts, the config messages published by MQTT DiscoveryStream must be retained by the MQTT broker. Ensure publish_retain: true is configured in your mqtt_discoverystream settings. If the integration does not default to this, manually configure this flag.

IF DUPLICATE ENTITIES OCCUR:

If you have encountered the issue of thousands of duplicate, non-functional entities, you will need to perform a comprehensive cleanup.

Stop the Source: Temporarily comment out your mqtt_discoverystream configuration in configuration.yaml on the publisher instance and restart Home Assistant to stop new duplicates from being generated.

Purge MQTT Retained Messages: Use an external MQTT client (such as MQTT Explorer) to connect to your MQTT broker. Navigate to the homeassistant/# topic (or your custom discovery_topic/#). Right-click the topic and select delete to clear all stale discovery messages.

Clean Home Assistant Registry: After clearing retained messages, restart Home Assistant Core on the affected instance. If entities still persist, you may need to manually edit Home Assistant's internal entity and device registries (located in /config/.storage/core.entity_registry and /config/.storage/core.device_registry via SSH or File Editor, with Home Assistant Core stopped).

Re-enable Correctly: Once cleaned, re-enable your mqtt_discoverystream configuration with the updated discovery_topic and restart Home Assistant.
